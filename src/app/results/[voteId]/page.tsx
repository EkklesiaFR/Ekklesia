...
import { ChevronLeft, Copy, ShieldCheck, Trophy } from 'lucide-react';
...
function ResultsDetailContent({ voteId }: { voteId: string }) {
  ...
  const ranking = vote.results?.fullRanking ?? [];
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      <header className="space-y-4">
        ...
        <h1 className="text-3xl md:text-5xl font-bold tracking-tight text-black">{vote.question}</h1>
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[10px] uppercase font-bold tracking-widest text-muted-foreground pt-2">
          <span>VoteId : {vote.id}</span>
          <span>État : {vote.state}</span>
...
export default function ResultsVoteDetailPage({ params }: { params: { voteId: string } }) {
  return (
    <RequireActiveMember>
      <MainLayout statusText="Archives">
        <ResultsDetailContent voteId={params.voteId} />
      </MainLayout>
    </RequireActiveMember>
  );
}