    ...
    <MainLayout statusText="Accès Restreint">
      <div className="flex flex-col items-center justify-center py-24 space-y-8 text-center animate-in fade-in duration-700">
        ...
        <header className="space-y-4">
          ...
          {showEmailWarning && <p className="text-sm font-bold text-primary bg-primary/5 p-4 mt-4 border border-primary/20">Vérifiez vos emails.</p>}
        </header>
...
export function RequireActiveMember({ children }: { children: ReactNode }) {
  ...
  if (user && !isEmailVerified && !isPublicPage) {
    return <RestrictedUI title="Email non vérifié" description="Validez votre email." icon={MailWarning} showEmailWarning={true} />;
  }

  if (user && member && !(isActiveMember || isAdmin) && !isPublicPage) {
    return <RestrictedUI title="Compte en attente" description="Votre adhésion doit être validée par un admin." />;
  }

  if (pathname.startsWith('/admin') && !isAdmin && !isPublicPage) {
    return <RestrictedUI title="Accès réservé" description="Droits admin requis." />;
  }
